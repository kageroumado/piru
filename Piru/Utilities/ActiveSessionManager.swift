...
final class ActiveSessionManager {
    ...
    private func recoverFromStore(container: ModelContainer) -> Bool {
        ...
        activeEntries = entries.map { entry in
            ...
            let matchedSubstance = SubstanceLibrary.resolveFull(snapshot.substance)
    ...
    func refreshEditedEntry(
        ...
        updateDose(
            ...
            entry: entry,
            substance: SubstanceLibrary.resolveFull(entry.substance),
            colorHex: colorHex,
            allColors: allColors,
        )
    }
    ...
    func buildSubstanceStates(colorMap: [String: String]) -> [ActiveSubstanceState] {
        activeEntries.compactMap { item in
            ...
            let substance = SubstanceLibrary.resolveFull(item.snapshot.substance)
