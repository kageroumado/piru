...
/// to *this* one's exposure, and by what enzyme mechanism. Kept beside its query
/// rather than in the main store file, which is at its line-count ceiling.
extension SubstanceStore {
    ...
    struct PKInteractionHit: Identifiable, Hashable {
        let id: Int64
    ...
    func pkInteractions(forSubstanceName name: String) -> [PKInteractionHit] {
        ...
        return Self.pkInteractionRows(substanceID: substanceID, db: substancesDB, order: enabledSourceOrder)
    }
    ...
    nonisolated static func pkInteractionRows(
        ...
        do {
            ...
            logger.error("pkInteractions(forSubstanceName:) failed: \(error.localizedDescription, privacy: .public)")
            return []
        }
    }
}
