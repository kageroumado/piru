...
struct TimelineStripDayContent: View {
    ...
    private func drawStrip(in context: inout GraphicsContext, size: CGSize, bubbleLeft: CGFloat, columnX: CGFloat) {
        ...
        // the strip continues.
        // Erased (destination-out), never painted: a skin's backdrop behind
        // the strip is a gradient, so a band of the flat background color
        // would read as a darker stripe across the top.
    ...
    /// Gap between a curve's peak and its name label.
    private static let curveLabelGap: CGFloat = 4
    private static let curveLabelPadding: CGFloat = 3
    ...
    private func drawCurveLabels(in context: inout GraphicsContext, axisX: CGFloat, curveWidth: CGFloat, bubbleLeft: CGFloat) {
        ...
        for series in day.series where !series.label.isEmpty {
            guard let peak = series.points.max(by: { $0.v < $1.v }) else { continue }
            ...
            placed.append(rect)
            context.fill(Path(roundedRect: rect, cornerRadius: 4), with: .color(plate))
            context.stroke(Path(roundedRect: rect, cornerRadius: 4), with: .color(series.color), lineWidth: 1)
            context.draw(text, in: rect.insetBy(dx: pad, dy: pad))
        }
    }
