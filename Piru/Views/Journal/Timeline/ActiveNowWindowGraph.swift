...
/// to the end of the last curve. Time is continuous here, exactly like the
/// full timeline screen the card opens: curves from earlier sessions flow into
/// the window instead of being cut at a session seam.
///
/// Rendered as a single evaluated ``TimelineWindowEvaluator/WindowPlot`` in one
/// `Canvas` (no tiles — the window fits the card), with wall-clock hour
/// gridlines, a heavier midnight boundary, baseline dose ticks, and the accent
/// now-marker. Evaluation and memoization ride ``TimelineRibbonModel`` — the
/// same engine as the full timeline, so the two can never disagree.
struct ActiveNowWindowGraph: View {
    ...
    /// Snapshot input window, matching the full ribbon: no curve outlives 48 h,
    ...
    /// Re-request identity for the window's evaluation, tile-style: the window
    ...
    /// One sample per ~3 minutes, matching the ribbon tiles' density, bounded
