...
struct MoleculeStructureView: View {
    ...
    private func draw(in context: inout GraphicsContext, size: CGSize) {
        ...
        for bond in structure.bonds {
            ...
            Self.appendBondLines(&sticks, from: start, to: end, order: bond.order, gap: Self.multiBondGap * scale)
        }
        context.stroke(
            sticks,
            with: .color(color),
    ...
    private static func appendBondLines(_ path: inout Path, from start: CGPoint, to end: CGPoint, order: Int, gap: CGFloat) {
