...
#
# The data/snapshots/* dumps are regenerated by every pipeline run and committed
# each time, at a few MB apiece; only the newest is ever read, and an older one
# is reproducible from the pipeline. piru-substances.sqlite is listed as a net
# rather than a routine — it is gitignored, so a blob of it in history means one
# slipped into a commit.
#
# Rewrites history so that, for each path below, every blob EXCEPT the one at
# HEAD is removed. Commits, messages and the graph are preserved; only the file
# content leaves the older commits, and HEAD's tree is left byte-identical.
#
# This is a history rewrite: it requires a force-push and invalidates existing
# clones and forks. Run it after a release, not mid-review.
#
# USAGE
