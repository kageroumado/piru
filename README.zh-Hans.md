...
<table>
  <tr>
    ...
    <td align="center"><img src=".github/piru-pharmacology-zh.png" alt="Piru 可卡因药理学卡片 —— 讲解其对多巴胺转运体的阻断作用，附再摄取抑制剂标签与血清素-多巴胺平衡滑块" width="380"><br><sub><b>药典</b> ・ 1,900+ 种物质的机制、结合与出处</sub></td>
  </tr>
</table>
...
> 曲线的起落、第二剂如何堆叠在第一剂之上、相互作用的危险窗口如何打开——而每一条主张，都注明来自
> 真实来源，方便你自己核实。这不是减害。这里没有人认为你需要被「减」。这是剂量的问题，而非善恶的问题。♡

---

Piru 是一款 iOS 剂量日志与参考工具，献给每一个会摄入某些东西的人——处方药、膳食补充剂或消遣性
物质——并希望为此留下一份诚实而私密的记录。两三下点击即可记录一剂；Piru 会叠加一条药代动力学曲线，
让你看清什么仍在活跃，在你叠加危险组合前发出警告，为耐受性的建立与消退建模，并用可追溯到具体研究的
引用来支撑它的药理内容。一切都留在你的设备上。没有账号，没有云端，也没有人在旁窥视。

## 功能

- **会自己作画的日志。** 每一剂都会化为一条药代动力学曲线。时间相近的剂量会归入一次**会话**，会话曲线图
  将各物质叠加呈现，让你一眼读懂整晚——什么正在达峰、什么正在消退、什么即将卷土重来。
- **1,900+ 种物质，条条有据。** 剂量阶梯（阈值 → 轻度 → 常规 → 强效 → 大剂量）、给药途径、
  起效/达峰/消退时长、半衰期、作用机制、受体结合与主观效应——按字段从 **17 个数据来源**解析，
  并逐一链回原始出处。
- **相互作用警告。** 基于药物类别的危险规则（单胺氧化酶抑制剂 + 兴奋剂、阿片 + 抑制剂、
  血清素能叠加等）在记录时即时浮现，并在时间轴上标出危险窗口。
- **耐受性与会话预测。** 一套耐受性模型追踪敏感度如何建立与恢复；一套经过校准的效应引擎预测
  兴奋剂或卡西酮类会话的走势——针对**苯丙胺、哌甲酯与 4-甲基甲卡西酮（2-/3-/4-MMC，即 mephedrone）**调校。
- **可调节的深度。** 从简洁到深入，全局一键切换——从朴素的剂量阶梯与顶层警告，
  直到受体结合表、偏向性激动与 CYP 代谢，引用细致到 DOI。
- **一套工具箱。** 半衰期计算器、容积式给药、苯二氮䓬与阿片等效换算、库存追踪，以及相互作用检查器。
...
多数记录类应用对所有物质都画同一条通用曲线便草草了事。Piru 做了两件我在其他剂量记录应用里从未见过的事。
...
Piru 内置一个离线 SQLite 物质库，收录 **1,913 种物质**。每一个字段——一段剂量范围、一段时长、
