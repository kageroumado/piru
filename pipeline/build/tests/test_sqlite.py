...
class TestSignatureGates(unittest.TestCase):
    ...
    def test_no_uncited_numeric_values(self):
        ...
        rows = self.db.execute(
            ...
            " UNION ALL "
            "SELECT s.canonical_name || '|functional|' || f.target || '|' || f.readout"
            ...
            " UNION ALL "
            "SELECT s.canonical_name || '|metabolism|' || m.enzyme"
            "  FROM metabolism m JOIN substances s ON s.id = m.substance_id"
            " WHERE m.citation_id IS NULL"
            "   AND COALESCE(m.fraction_of_clearance_pct, m.metabolite_half_life_min,"
            "                m.metabolite_potency_vs_parent_pct, m.formation_fraction_pct) IS NOT NULL"
            " UNION ALL "
            ...
            " UNION ALL "
            "SELECT s.canonical_name || '|conc_effects|' || c.effect"
            "  FROM concentration_effects c JOIN substances s ON s.id = c.substance_id"
            " WHERE c.citation_id IS NULL"
            "   AND COALESCE(c.threshold, c.peak_effect) IS NOT NULL"
            " UNION ALL "
            "SELECT s.canonical_name || '|ddi|' || d.with_substance"
            "  FROM drug_interactions_pk d JOIN substances s ON s.id = d.substance_id"
            " WHERE d.citation_id IS NULL"
            "   AND d.ki_um IS NOT NULL"
            " UNION ALL "
            "SELECT s.canonical_name || '|off_targets|' || o.target"
            "  FROM off_targets o JOIN substances s ON s.id = o.substance_id"
            " WHERE o.citation_id IS NULL"
            "   AND o.ki_or_ic50_nm IS NOT NULL"
        ).fetchall()
