...
def _resolved(db, table, value_col, sid, priority):
    ...
    rows = db.execute(
        ...
        f"JOIN sources src ON src.id = t.source_id WHERE t.substance_id = ?",
        (sid,),
    ).fetchall()
    ...
    for s in db.execute("SELECT * FROM substances ORDER BY canonical_name COLLATE NOCASE"):
        ...
        dose_routes = [
            ...
            for r in db.execute(
                ...
                "OR strong_lower IS NOT NULL OR heavy IS NOT NULL)",
                (sid,),
            )
        ]
        all_routes = sorted(
            {
                ...
                for r in db.execute(
                    ...
                    "UNION SELECT route FROM protocol_dosing WHERE substance_id=:i",
                    {"i": sid},
                )
            }
        )
        has_duration = (
            db.execute("SELECT 1 FROM durations WHERE substance_id=? LIMIT 1", (sid,)).fetchone()
            is not None
        )
        has_protocol = (
            db.execute(
                "SELECT 1 FROM protocol_dosing WHERE substance_id=? LIMIT 1", (sid,)
            ).fetchone()
            is not None
        )
        ...
        effects = [
            r["text"]
            for r in db.execute("SELECT DISTINCT text FROM effects WHERE substance_id=?", (sid,))
        ]
        refs = [
            r["label"]
            for r in db.execute(
        ...
        rows.append(
            {
                ...
                "tags": tags,
                "default_route": dose_routes[0]
                if dose_routes
                else (all_routes[0] if all_routes else ""),
...
def main() -> int:
    ...
    OUT_JSON.write_text(
        json.dumps([{**e, "data_gaps": gaps_for(e)} for e in rows], indent=2, ensure_ascii=False)
        + "\n"
    )
    ...
    print(
        f"  with protocol:     {sum(1 for e in rows if e.get('has_protocol_dosing'))}",
        file=sys.stderr,
    )
