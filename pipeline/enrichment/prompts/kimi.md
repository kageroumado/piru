...
3. Merge into `data/intermediate/substances-bundled.json` via the same `SubstanceDeduplicator` the app uses at runtime — Piru's existing curated values win on conflict; Kimi enrichments fill gaps only.
