...
| `localized-names.json` | `localized_names` (+ locale-tagged `aliases`) | the title a substance takes in each app language (es, zh-Hans, zh-Hant) |
...
| **class prior** | The log-normal spread of the same column across the substance's class peers, leave-one-out, as a robust z. Cascades class context → drug class → interaction class → category. A compound with no class, or a class of one, gets **no** signal and the cell says `no_class_signal` rather than inventing one. A peer only counts toward `min_class_members` when its own consensus rests on more than the sources arguing in *this* cell: heroin's intravenous come-up has five classical-opioid peers, three of them a number drug.community published alone, and a prior fitted on those rules for drug.community against the two sources contradicting it. Below the floor the cell says `class_signal_thin` and the prior scores nobody. |
