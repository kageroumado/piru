#!/usr/bin/env python3
"""Build a flat tabular export of every substance Piru ships with.

Merges:
  - Piru/Data/substances-bundled.json   (curated overlay, Substance schema)
  - Piru/Data/drug-community-data.json  (drug.community dump, their schema)

Outputs to ./Exports/:
  - piru-database.csv   (one row per compound, semicolon-delimited multi-value cells)
  - piru-database.json  (same data, structured)
  - piru-database-gaps.csv (only rows with data gaps — convenience for crawlers)

Run from the repo root:
    python3 Exports/build-database-export.py
"""
...
CURATED = REPO / "Piru/Data/substances-bundled.json"
DRUG_COMMUNITY = REPO / "Piru/Data/drug-community-data.json"
OUT_DIR = REPO / "Exports"
OUT_CSV = OUT_DIR / "piru-database.csv"
OUT_JSON = OUT_DIR / "piru-database.json"
OUT_GAPS_CSV = OUT_DIR / "piru-database-gaps.csv"
