...
Run from the repo root:
    python3 Exports/compare_to_pw.py > Exports/pw-divergence.md
"""
...
for r in db.execute("""
    ...
    FROM substances s
""").fetchall():
    score = (r["aliases"] or 0) * 0.5 + (r["sources_with_doses"] or 0) * 2 + (8 if r["is_common"] else 0)
    popularity[r["sid"]] = (r["name"], score)

# ── PW substances ────────────────────────────────────────────────────────
pw_substance_ids = {
    r["substance_id"] for r in db.execute("""
...
for sid in sorted(pw_substance_ids):
    ...
    for route, rows in by_route_dose.items():
        ...
        def representative(row):
            return (row["common_upper"] or row["common_lower"]
                    or row["strong_upper"] or row["light_upper"]
                    or row["heavy"])
        ...
        unit_mismatch = (pw_factor is not None and rs_factor is not None
        ...
        if pw_factor is not None and rs_factor is not None and pw_val and rs_val:
            ...
            if ratio_significant(pw_mg, rs_mg) or unit_mismatch:
                findings.append({
                    ...
                    "type": "dose", "route": route,
                    "pw": fmt_dose(pw_row["threshold"], pw_row["light_lower"], pw_row["light_upper"],
                                   pw_row["common_lower"], pw_row["common_upper"],
                                   pw_row["strong_lower"], pw_row["strong_upper"],
                                   pw_row["heavy"], pw_row["unit"]),
                    "resolved": fmt_dose(resolved["threshold"], resolved["light_lower"], resolved["light_upper"],
                                         resolved["common_lower"], resolved["common_upper"],
                                         resolved["strong_lower"], resolved["strong_upper"],
                                         resolved["heavy"], resolved["unit"]),
                    "winner": resolved["source_slug"],
                    "ratio_note": fmt_ratio(pw_mg, rs_mg) if not unit_mismatch
    ...
    for route, by_source in by_route_dur.items():
        ...
        ph_check = "total" if "total" in pw_phases and "total" in rs_phases \
        ...
        if resolved_hl is not None and resolved_hl["source_slug"] != "psychonautwiki":
            if ratio_significant(pw_hl["half_life_minutes"], resolved_hl["half_life_minutes"]):
                findings.append({
                    ...
                    "ratio_note": fmt_ratio(pw_hl["half_life_minutes"], resolved_hl["half_life_minutes"]),
                })
...
print(f"Out of {len(pw_substance_ids)} substances with PW data, "
      f"{len({f['name'] for f in findings})} have at least one significant divergence "
      f"({len(findings)} total findings).\n")
print("Significant = field value differs by ≥2× from PW after unit conversion. "
...
for f in findings:
    ...
    pw  = f["pw"][:60]
    rs  = f["resolved"][:60]
    print(f"| {f['score']:.0f} | {name} | {f['type']} | {f['route']} | {f['winner']} | {pw} | {rs} | {f['ratio_note']} |")
