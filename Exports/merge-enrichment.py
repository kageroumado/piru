...
Inputs:
  Exports/enrichment/<group-slug>.json — each agent's output.
  Schema: an array of per-compound records plus class-context objects
  (distinguishable by `is_class_context: true`).

Outputs:
  Exports/enrichment-merged.json     — flat list of all per-compound records
  Exports/enrichment-class-context.json — list of class-context objects
  Exports/enrichment-coverage.csv    — one row per substance, columns indicate
...
IN_DIR = REPO / "Exports/enrichment"
OUT_MERGED = REPO / "Exports/enrichment-merged.json"
OUT_CONTEXT = REPO / "Exports/enrichment-class-context.json"
OUT_COVERAGE = REPO / "Exports/enrichment-coverage.csv"
