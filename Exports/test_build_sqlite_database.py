"""Regression tests for drug.community dose-string parser in build-sqlite-database.py.
...
Run from the repo root:
    python3 Exports/test_build_sqlite_database.py
"""
...
# The module filename contains hyphens, so standard import won't work.
_spec = importlib.util.spec_from_file_location(
    "bsd", Path(__file__).parent / "build-sqlite-database.py"
)
...
class TestBuiltDatabaseInvariants(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        sqlite_path = Path(__file__).parent.parent / "Piru/Data/piru-substances.sqlite"
        if not sqlite_path.exists():
            raise unittest.SkipTest("piru-substances.sqlite not built; run Exports/build-sqlite-database.py first")
