...
struct MechanismOfActionTests {
    ...
    @Test("Releaser cathinones bind transporters as releasing agents, never as modulators")
    func cathinoneReleasersAreReleasers() {
        for name in ["mephedrone", "4-mmc", "3-mmc", "2-mmc", "methylone", "methcathinone"] {
            ...
            for t in transporters {
                #expect(t.action == .releasingAgent,
                        "\(name) \(t.target) should be a releasingAgent, got \(t.action.rawValue)")
            }
        }
    }

    @Test("Pyrovalerone cathinones are DAT/NET reuptake inhibitors, not releasers")
    func pyrovaleronesAreReuptakeInhibitors() {
        for name in ["mdpv", "α-pvp", "a-pvp", "α-php"] {
            ...
            for t in transporters {
                #expect(t.action == .reuptakeInhibitor,
                        "\(name) \(t.target) should be a reuptakeInhibitor, got \(t.action.rawValue)")
            }
        }
    }

    @Test("Mitragynine and kratom are partial MOR agonists with adrenergic activity")
    func mitragyninePartialAgonist() {
    ...
    @Test("Measured DB bindings beat the category's generic Modulator placeholder")
    func dbBindingsBeatCategoryPlaceholder() {
        ...
        #expect(resolved != nil)
        #expect(resolved?.bindings.allSatisfy { $0.action == .releasingAgent } == true,
    ...
    @Test("A curated per-name entry's bindings win over a thinner DB panel")
    func handEntryBindingsWinOverDB() {
        ...
        #expect(resolved?.bindings.contains { $0.target.localizedCaseInsensitiveContains("adrenergic") } == true,
    ...
    @Test("A real DB summary is preferred verbatim over fallbacks")
    func dbSummaryWins() {
    ...
    @Test("An unknown substance falls back to its category mechanism (the floor)")
    func unknownFallsBackToCategory() {
    ...
    @Test("Mephedrone resolves to a releaser mechanism, not generic Modulator")
    @MainActor
    func mephedroneIntegration() {
        ...
        #expect(transporters.allSatisfy { $0.action == .releasingAgent },
    ...
    @Test("Mitragynine resolves to partial agonist end-to-end, never 'Full Agonist'")
    @MainActor
    func mitragynineIntegration() {
    ...
    @Test("Substances with measured bindings expose them even without a summary row")
    @MainActor
    func bindingsSurfaceWithoutSummary() {
        ...
        #expect(sub.mechanismOfAction != nil,
                "Mephedrone's DB mechanism should be non-nil (bindings present)")
        #expect(sub.mechanismOfAction?.bindings.isEmpty == false,
