...
struct SubstanceCodableTests {
    ...
    @Test("CodableRange converts to and from ClosedRange")
    func codableRangeConversion() {
    ...
    @Test("DoseRange round-trips through JSON")
    func doseRangeRoundTrip() throws {
    ...
    @Test("Substance round-trips through JSON")
    func substanceRoundTrip() throws {
    ...
    @Test("Substance with minimal fields round-trips")
    func substanceMinimalRoundTrip() throws {
        let original = Substance(
            name: "Minimal",
            aliases: [],
            category: .other,
    ...
    @Test("Full library encodes without error")
    func encodeFullLibrary() throws {
        let data = try JSONEncoder().encode(SubstanceLibrary.all)
        #expect(data.count > 0)
    }

    @Test("Full library round-trips with same count and names")
    func roundTripFullLibrary() throws {
        let data = try JSONEncoder().encode(SubstanceLibrary.all)
        let decoded = try JSONDecoder().decode([Substance].self, from: data)
