...
struct DueNowSlotTests {
    ...
    func `A passed, uncovered slot is due`() {
        let slots = DueNowSlot.derive(items: [med("Sertraline", times: [480])], loggedToday: [:], now: noon)
    ...
    func `A slot inside the lead window is due; beyond it is not`() {
        ...
        let slots = DueNowSlot.derive(items: [soon, later], loggedToday: [:], now: noon)
    ...
    func `Covered slots are skipped — earliest-first assignment`() {
        ...
        let one = DueNowSlot.derive(items: [twice], loggedToday: [twice.identityKey: 1], now: noon)
        #expect(one.map(\.slotMinutes) == [11 * 60])
        // Two doses cover both.
        let none = DueNowSlot.derive(items: [twice], loggedToday: [twice.identityKey: 2], now: noon)
        #expect(none.isEmpty)
    }
    ...
    func `An anytime med is due all day until taken, and sorts behind timed slots`() {
        ...
        let slots = DueNowSlot.derive(items: [vitamin, ssri], loggedToday: [:], now: noon)
        #expect(slots.map(\.slotMinutes) == [480, nil])
        let taken = DueNowSlot.derive(items: [vitamin], loggedToday: [vitamin.identityKey: 1], now: noon)
    ...
    func `PRN meds and off-day schedules never appear`() throws {
        ...
        let slots = DueNowSlot.derive(items: [prn, weekly], loggedToday: [:], now: noon)
    ...
    func `Quiet flag rides through for the strip's supplement fold`() {
        ...
        let slots = DueNowSlot.derive(items: [supp], loggedToday: [:], now: noon)
