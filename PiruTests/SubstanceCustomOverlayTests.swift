...
struct SubstanceCustomOverlayTests {
    ...
    @Test
    func `No-op when custom has no duration`() {
    ...
    @Test
    func `Custom duration replaces matching route's duration`() {
        let library = Self.libraryStub()
    ...
    @Test
    func `Adds a new route when library has none for the custom's defaultRoute`() {
        let library = Self.libraryStub()
        let custom = CustomSubstanceEntry(
    ...
    @Test
    func `Preserves library category, half-life, effects, tags`() {
        let library = Self.libraryStub()
        let custom = CustomSubstanceEntry(
            name: "2-MMC", defaultRoute: .oral, duration: Self.customDuration(),
        )
    ...
    @Test
    func `Appends user-defined source attribution`() {
        let library = Self.libraryStub()
        let custom = CustomSubstanceEntry(
            name: "2-MMC", defaultRoute: .oral, duration: Self.customDuration(),
        )
    ...
    @Test
    func `Source attribution not duplicated when applied twice`() {
        let library = Self.libraryStub()
        let custom = CustomSubstanceEntry(
            name: "2-MMC", defaultRoute: .oral, duration: Self.customDuration(),
        )
        ...
        #expect(twice.sources.count(where: { $0 == CustomSubstanceEntry.userDefinedSource }) == 1)
    }

    @Test
    func `Display-name override sets displayTitle, keeps canonical name`() {
        let library = Self.libraryStub()
    ...
    @Test
    func `Blank display name does not override`() {
        let library = Self.libraryStub()
    ...
    @Test
    func `Dose-range override replaces matching route's doses`() {
        let library = Self.libraryStub()
        let custom = CustomSubstanceEntry(
    ...
    @Test
    func `Half-life override replaces library half-life`() {
        let library = Self.libraryStub()
    ...
    @Test
    func `Empty override is a true no-op`() {
        let library = Self.libraryStub()
    ...
    @Test
    func `Pre-1.4 entries decode with nil overrides (backward compatible)`() throws {
        ...
        var obj = try #require(JSONSerialization.jsonObject(with: JSONEncoder().encode(modern)) as? [String: Any])
        for key in ["displayName", "doses", "halfLifeMinutes"] {
    ...
    @Test
    func `resolvedDisplayName prefers the personal label`() {
    ...
    @Test
    func `ActiveSubstanceState.from succeeds for the overlaid substance`() {
        let library = Self.libraryStub()
        let custom = CustomSubstanceEntry(
            name: "2-MMC", defaultRoute: .oral, duration: Self.customDuration(),
        )
