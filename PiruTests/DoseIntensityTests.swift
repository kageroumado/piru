...
struct DoseIntensityTests {
    ...
    @Test
    func `Within substance: intensity scales proportionally with amount`() {
    ...
    @Test
    func `Plat-1 DXM (light) renders much shorter than 75g alcohol (overshoot heavy)`() {
    ...
    @Test
    func `Heavy-overdose saturates at 1.0`() {
        #expect(intensity(10_000, dxmLike) == 1.0)
        #expect(intensity(200, alcoholLike) == 1.0) // 5× heavy
    }

    @Test
    func `Monotonic with amount — always`() {
        let values = [50, 100, 150, 250, 400, 550, 700, 1_000].map { intensity(Double($0), dxmLike) }
    ...
    @Test
    func `Floor at 0.05 so tiny doses still show a visible nub`() {
        #expect(intensity(0, dxmLike) >= 0.05 - 0.0001)
        #expect(intensity(0, dxmLike) <= 0.05 + 0.0001)
    }

    @Test
    func `Nil dose range returns neutral mid-common, not full height`() {
    ...
    @Test
    func `Falls back to strong upper when heavy is missing`() {
    ...
    @Test
    func `Falls back to common × 1.5 when only common is defined`() {
